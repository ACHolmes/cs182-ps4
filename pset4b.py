...
class TensorFlowMNISTClassifier(MNISTClassificationModel):
    def __init__(self):
        ...
        class NeuralNet(tf.Module):
            def __init__(self):
                # Have to set these as attributes so that we can access
                # within the call method
                # 784 by 100 matrix of weights, so that each 784 pixel contributes
                # uniquely to each of the 100 nodes in hidden layer
                self.w0 = tf.Variable(tf.random.uniform([784, 100]))
                self.b0 = tf.Variable(tf.random.uniform([100]))

                self.w1 = tf.Variable(tf.random.uniform([100, 10]))
                self.b1 = tf.Variable(tf.random.uniform([100]))

            def __call__(self, x0):
                # Finding first layer output, x_0 W_0 + b_0
                # x_0 is a row vector, W_0 is a matrix, gives out a column vector of size 100
                x1 = tf.nn.sigmoid(x0 @ self.w0 + self.b0)
                # Second layer output using previous layer's output
                return tf.nn.softmax(x1 @ self.w1 + self.b1)


        self.neural_net_model = NeuralNet()
